        ...
        <>
            ...
            <div className='h-fit w-full py-10 bg-white'>
                <div className='flex justify-center mb-1 '>
                    <input className='h-[3rem] w-[20rem] border border-3 border-red-900' type='text' placeholder='search me' value={searchQuery}
                        onChange={e => setSearchQuery(e.target.value)} />
                </div>
                <div className='h-full w-11/12 m-auto bg-transparent gap-3 grid grid-cols-3 '>
                    {filteredData.map((value, index) => (
                        <div key={index} className='w-auto h-[210px] border border-3 border-red-800 text-center bg-yellow-400 text-black font-bold rounded relative'>
                            <img className='h-20 w-20' src={value.image} alt={value.technology} />
                            <div>
                                <h4 className='text-gray-500 text-lg'>{value.technology}</h4>
